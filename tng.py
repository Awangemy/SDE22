...
import tempfile
import os
from datetime import datetime, timedelta  # Diperlukan untuk tarikh
...
conn_str = "postgresql://postgres.uonphwbbgemsvqzrdcwp:Ika_15050107@aws-1-ap-southeast-1.pooler.supabase.com:5432/postgres"
    ...
    try:
        ...
        if "TransactionDateTime" in df.columns:
            ...
            df["TransactionDateTime"] = pd.to_datetime(
                df["TransactionDateTime"], dayfirst=True, errors="coerce"
            )
...
async def download_filtered():
    save_folder = r"C:\Users\awang\Compiler\filter"
    file_path = os.path.join(save_folder, "filtered_new_rows.csv")
    if not os.path.exists(file_path):
        return {"status": "error", "message": "Tiada fail hasil tapis ditemui."}
    return FileResponse(file_path, filename="filtered_new_rows.csv", media_type="text/csv")
...
def get_tng_data(
    ...
    try:
        # === Guna connection yang sama ===
        engine = create_engine(conn_str)

        # Tetapkan tarikh default
        ...
        if filter_6am:
            ...
            end_dt = datetime.strptime(end_date, "%Y-%m-%d") + timedelta(hours=6)
        else:
    ...
    try:
        with engine.connect() as conn:
            result = conn.execute(text("""
                ...
                ORDER BY "PlazaNo";
            """))
            plaza_list = [row[0] for row in result]
